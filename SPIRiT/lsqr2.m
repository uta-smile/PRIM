function [x,info,flag,relres,iter,resvec,lsvec] = lsqr2(A,b,tol,maxit,M1,M2,x0,snrinput,varargin)
%LSQR   LSQR Method.
...
%   $Revision: 1.6.4.8 $ $Date: 2008/10/31 06:20:42 $
...
% Finally, make sure that maxit is not larger than n.
maxit = min(n,maxit);
...
for ii = 1 : maxit
    ...
    res = snrinput.y;
    res(snrinput.idx_nacq) = x;    
    im = ifft2c(res);
     
    info.snr(ii) = snr(im,snrinput.F0);
    info.time(ii) = cputime()-t0;
end                            % for ii = 1 : maxit
