...
% 
% fprintf('RMSE: %.8f, time=%.2fs\n', RMSE(im_cgSPIRiT, data), t_cgSPIRiT);
...
% 
% fprintf('RMSE: %.8f, time=%.2fs\n', RMSE(im_cssense, data), t_cssense);
...
input.no = 10; % number of iterations for outer loop, may need to change for different datasets
...
fprintf('RMSE: %.8f, time=%.2fs\n', RMSE(im_pcg_jtv, data), t_pcg_jtv);
