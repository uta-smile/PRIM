...
sz=options.sz; Phi=options.Phi; f0=options.f0; 
%----------------------------------------------------------------------
% Check arguments. 
%----------------------------------------------------------------------
...
%     end % Act on exit conditions.
   if  iter >= maxIts
    ...
    for t=1:T,
        fest((t-1)*sz(1,3)+1:t*sz(1,3),1)=Phi'*xBest((t-1)*sz(1,3)+1:t*sz(1,3),1);
    end
    Trace_SNR(iteration,1)=snr(fest, f0(:));
    Trace_Time(iteration,1)=cputime-tt;
end % while 1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
info.Trace_Time=Trace_Time;
info.Trace_SNR=Trace_SNR;
% Print final output.
switch (stat)
   case EXIT_OPTIMAL
...
function z = Aprod(x,mode)
   ...
   if mode == 1
      ...
      if iscell(A)
          T=length(A);
          for t=1:T,
              start=sz(t,1); stop=sz(t,2);pn=sz(t,3);
              z(start:stop,1)=A{t}*x((t-1)*pn+1:t*pn,1);
          end          
      else
          if   explicit, z = A*x;
          else           z = A(x,1);
   ...
   elseif mode == 2
      ...
      if iscell(A)
          ...
          for t=1:T,
              start=sz(t,1); stop=sz(t,2); pn=sz(t,3);
              z((t-1)*pn+1:t*pn,1)=A{t}'*x(start:stop,1);
          end          
